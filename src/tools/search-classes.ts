	...
	server.registerTool(
		'search_classes',
		{
			...
			inputSchema: {
				...
				jars: PARAMS.jars,
				scope: PARAMS.scope,
				query: z.string().describe('Glob pattern to match against fully-qualified class names. * matches one segment, ** crosses package boundaries.'),
				caseSensitive: z.boolean().optional().describe('Case-sensitive matching (default: false)'),
				kind: z.array(z.enum(['class', 'interface', 'enum', 'record', '@interface'])).optional().describe('Filter by class type'),
		...
		async ({ query, caseSensitive, kind, jars, scope, offset, limit, project, details }) => {
			logger.debug('search_classes called', { project, query, caseSensitive, kind, jars, offset, limit });
			...
			const response = await searchClasses(
				{ query, caseSensitive, kind, offset, limit },
				resolvedDeps,
