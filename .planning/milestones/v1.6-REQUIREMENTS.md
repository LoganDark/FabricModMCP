# Requirements Archive: v1.6 Windows Support
...
- [x] **WIN-01**: JDT LS spawns successfully on Windows when Java home is supplied via `--java-home`, `JAVA_HOME`, or discovery — `java.exe` resolution works for absolute paths so `child_process.spawn` (which doesn't honor PATHEXT) succeeds. *Validated end-to-end by Phase 39-04 matrix rows 1-4 on Windows 11 (all 4 Java-discovery slots produced live JDT LS sessions; cross-jar find_definition succeeded under each).*
...
- [x] **UNIX-01**: Existing Unix `detectJava` / `findJdtLs` behavior is byte-identical for users who don't set `org.gradle.java.home` (no behavioral change on Linux/macOS). *Validated by Phase 35 (every helper's Unix branch returns v1.5 literals verbatim) and Phase 39-05 UNIX-03 sweep (872 macOS tests pass, exit 0).*
...
| REQ-ID | Phase | Status |
|--------|-------|--------|
| WIN-01 | Phase 35 | Complete |
| WIN-02 | Phase 38 | Complete |
| WIN-03 | Phase 36 | Complete |
| WIN-04 | Phase 36 | Complete |
| WIN-05 | Phase 36 | Complete |
| WIN-06 | Phase 36 | Complete |
| WIN-07 | Phase 36 | Complete |
| JAVA-01 | Phase 37 | Complete |
| JAVA-02 | Phase 37 | Complete |
| JAVA-03 | Phase 37 | Complete |
| JAVA-04 | Phase 37 | Complete |
| JAVA-05 | Phase 37 | Complete |
| UNIX-01 | Phase 35 | Complete |
| UNIX-02 | Phase 36 | Complete |
| UNIX-03 | Phase 39 | Complete |
